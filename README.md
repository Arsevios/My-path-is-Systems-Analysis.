👋 Hi there! I'm Arsevios — Aspiring Systems Analyst

I am a student at the very beginning of my IT career journey. This repository is my learning journal where I compile everything I study and practice in the field of Systems Analysis. My goal is to structure my knowledge and visually demonstrate my professional growth.


🗺️ My Path to Systems Analysis

🎯 My Goal
To become a sought-after Systems Analyst who helps businesses and development teams speak the same language. My long-term aim is to work remotely on international projects (US time zones).

📚 What I'm Currently Learning
   Core Fundamentals: Software Development Life Cycle (SDLC), Methodologies (Agile, Scrum, Waterfall).
   Requirements Engineering: How to properly elicit, analyze, and document requirements (User Stories, Use Cases, SRS).
   Process & Data Modeling: BPMN, UML (Use Case, Sequence, Class Diagrams), ERD.
   Tools: Figma, Draw.io, Miro, Jira, Confluence, Postman.
   Technical Foundation: SQL (basic queries), Understanding of APIs (REST, SOAP).

🧩 My Projects & Pet Projects and 🛠️ My Tech Stack
Pretty bare right now, but I'll be learning and creating more soon!


📞 Contact Me
LinkedIn: https://www.linkedin.com/in/arsevios-grace-634727346/?lipi=urn%3Ali%3Apage%3Ad_flagship3_profile_view_base_contact_details%3BmN%2F4cmX2SWetEQLkM0Derw%3D%3D


I am always open to communication, advice, and collaboration! I welcome any feedback or recommendations.
